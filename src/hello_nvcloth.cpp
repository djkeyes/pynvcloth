...
using Factory = unique_ptr<nv::cloth::Factory, decltype(&NvClothDestroyFactory)>;

struct FabricDeleter {
	void operator()(nv::cloth::Fabric* const f) {
		f->decRefCount();
	}
};
...
struct ClothDeleter {
	void operator()(nv::cloth::Cloth* const c) {
		NV_CLOTH_DELETE(c);
	}
};
...
void run_simple_simulation() {
	...
	Fabric fabric(NvClothCookFabricFromMesh(factory.get(), mesh_desc, gravity, &phase_types, false));

	Cloth cloth(factory->createCloth(nv::cloth::Range<physx::PxVec4>(&particle_positions[0], &particle_positions[0] + particle_positions.size()), *fabric));
